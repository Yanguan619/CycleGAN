...
class Visualizer:
    ...
    def display_current_results(self, visuals, epoch, save_result):
        ...
        if self.use_html and (save_result or not self.saved):  # save images to an HTML file if they haven't been saved.
            ...
            # update website
            webpage = html_.HTML(self.web_dir, 'Experiment name = %s' % self.name, refresh=1)
...
class VisualizerCallback(Callback):
    ...
    def on_train_epoch_end(self, epoch, save_result):
        self.visualizer.reset()
        visual_ret = {}
        for name in self.visal:
            visual_ret[name] = getattr(self, name)
        self.visualizer.display_current_results(visual_ret, epoch, save_result)
